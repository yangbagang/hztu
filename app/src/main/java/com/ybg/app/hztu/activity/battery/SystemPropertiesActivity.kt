...
class SystemPropertiesActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        batteryItemAdapter = BatteryItemAdapter(this@SystemPropertiesActivity)
        ...
        rv_battery_list.layoutManager = LinearLayoutManager(this@SystemPropertiesActivity, layoutManager, false)
        rv_battery_list.itemAnimator = DefaultItemAnimator()
        rv_battery_list.addItemDecoration(SpaceItemDecoration(2))

        rl_fresh_layout.setRefreshViewHolder(BGANormalRefreshViewHolder(this@SystemPropertiesActivity, true))
        ...
        detailButton.setOnClickListener {
            if (battery != null) {
                SystemDetailActivity.start(this@SystemPropertiesActivity, battery!!)
            }
        }
    ...
    private fun getSystemAddress(lac: Int, cid: Int, type: Int) {
        SendRequest.getLocation(this@SystemPropertiesActivity, userApplication.token, lac, cid, type, object :
                JsonCallback(){
    ...
    private fun getBatteryList() {
        ...
        SendRequest.getBatteryDataByUid(this@SystemPropertiesActivity, userApplication.token, battery!!.uid,
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            ...
            R.id.action_chart -> {
                if (battery != null) {
                    SystemLineChartActivity.start(this@SystemPropertiesActivity, battery!!)
                }
            }
            R.id.action_detail -> {
                if (battery != null) {
                    SystemHistoryActivity.start(this@SystemPropertiesActivity, battery!!)
                }
            }
        }
    ...
    private val systemItemClickListener = object : RecyclerBaseAdapter.OnItemClickListener {
        override fun onItemClick(position: Int) {
            ...
            BatteryDetailActivity.start(this@SystemPropertiesActivity, battery)
        }
    }

    companion object {
        fun start(context: Context, battery: Battery) {
            val starter = Intent(context, SystemPropertiesActivity::class.java)
